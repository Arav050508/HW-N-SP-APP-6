...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addTask(name, subject, description, date, type, reminderInterval, groupMembers) {
        ...
        const id = new Date().getTime().toString(); // Generate a unique ID using the current timestamp
    
        const event = {
            id: id, // Ensure the ID is a string
            title: `${name} - ${subject}`,
            start: dueDateTime,
    ...
    function storeTask(name, subject, description, date, type, reminderInterval, groupMembers) {
        ...
        tasks.push({ id, name, subject, description, date, type, reminderInterval, groupMembers });
    ...
    deleteTaskButton.onclick = function() {
        const eventId = modal.dataset.eventId; // Get the event ID from modal dataset
